...
const SystemStatus = () => {
  ...
  const getServiceStatus = (serviceStatus) => {
    if (!serviceStatus) return { color: 'text-gray-500', icon: <ArrowPathIcon className="h-4 w-4 animate-spin" />, text: 'Loading' };
    ...
    if (isIdle || status === 'recovering_from_idle') return { color: 'text-yellow-600', icon: <ArrowPathIcon className="h-4 w-4 animate-spin" />, text: 'Loading' };
  ...
  const dbServiceStatus = getServiceStatus(dbStatus);
  const redisServiceStatus = getServiceStatus(redisStatus);
